...
_template = function(text, data, settings) {
	settings = _.defaults(settings || {}, _tmpl_settings);
...
var _re = /{%\s*define\s+(\S+)\s+%}([\s\S]*?){%\s*end\s+define\s*%}/g;

var template = function(text, data, settings) {
	var result = {};
	while(1) {
		var match = _re.exec(text);
		if (!match)
			break;
		result[match[1]] = _template(match[2], data, settings);
	}
	return result;
};
